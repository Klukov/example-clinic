...
class VisitRepositoryImpl implements VisitRepository, DoctorAvailabilityRepository {
    ...
    public Optional<Visit> findVisit(VisitId visitId) {
        return visitJpaRepository.findById(visitId.getValue()).map(VisitDao::toDomain);
    }
    ...
    public Visit update(Visit visit) {
        var existingVisit =
                visitJpaRepository
                        .findById(visit.getId().getValue())
                        .map(VisitDao::toBuilder)
                        .orElseThrow(() -> getNotFoundVisitException(visit))
    ...
    public Set<DoctorId> findAllAvailableDoctors(LocalDateTime from, LocalDateTime to) {
        return visitJpaRepository.findAllVisits(from, to, List.of(VisitStatus.FREE), null).stream()
                .map(VisitDao::getDoctorId)
                .map(DoctorId::of)
                .collect(Collectors.toSet());
    }

    private static ClinicRuntimeException getNotFoundVisitException(Visit visit) {
        return new ClinicRuntimeException(
                String.format("Visit with id %s do not exists", visit.getId()));
    }
}
